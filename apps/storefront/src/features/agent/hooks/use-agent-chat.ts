...
export function useAgentChat() {
	...
	const toolResultQueueRef = useRef<ToolResult[]>([]);
			...
			try {
				...
				for await (const event of stream) {
					if (event.type === "text") {
						...
						if (toolName === "getCart") {
						...
						} else if (toolName === "listProducts") {
							const { data: products } = JSON.parse(event.content) as { data: AgentProduct[] };
							...
							const { found, product } = JSON.parse(event.content) as {
								found: boolean;
								product: AgentProductDetail | null;
							};
							if (found && product) {
								toolResultQueueRef.current.push({
									id: crypto.randomUUID(),
						...
						} else if (toolName === "startCheckout") {
							const { success, order } = JSON.parse(event.content) as { success: boolean; order?: OrderData };
							if (success && order) {
								toolResultQueueRef.current.push({
									id: crypto.randomUUID(),
									role: "tool_result",
						...
						setActiveTool(null);
						if (toolResultQueueRef.current.length > 0) {
							const queued = toolResultQueueRef.current;
							toolResultQueueRef.current = [];
							setMessages((prev) => {
								const idx = prev.findIndex((m) => m.id === assistantId);
								if (idx === -1) return [...prev, ...queued];
								return [...prev.slice(0, idx), ...queued, ...prev.slice(idx)];
							});
						}
					}
				}
